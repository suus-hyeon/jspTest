...
<div class="container">
	...
	<div>
		...
		1-2.길이(members배열) : ${fn:length(members)}<br/>
	  ...
	  <c:set var="first" value="${fn:indexOf(message, 'o')}" /><br/>
	  <c:set var="cut" value="${fn:substring(message, first+1, fn:length(message))}"/><br/>
	  <c:set var="gogo" value="${first + 1 + fn:indexOf(cut, 'o')}"/><br/>
	  숙제 두번째 'o'의 위치는? : ${gogo}
	  
	 <c:set var="good" value="${fn:split(message, 'o')}"/><br/>
	 <c:set var="count" value="${fn:length(good)-1}"/><br/>
	 'o'의 전체 개수는? : ${count}<br/><br/>
	 
	 'o'의 전체 위치는 ? : <br/>
	 <c:forEach var="i" begin="1" end="${fn:length(message)}" varStatus="st">
	   <c:if test="${fn:substring(message,st.count-1,st.count)=='o'}">
	     ${st.count - 1} 
	   </c:if>
	 </c:forEach>
	 <br/><br/>
	  ...
	  <br/>
	  6.치환(replace())<br/>
	  str변수안의 'o'를 '0'로 치환 ${fn:replace(message, 'o', '0')}<br/>
	  <br/>
	  7.포함(contains()) : <br/>
