...
<div class="container">
    ...
    <ul class="pagination" style="justify-content:center;">
      <c:if test="${pag > 1}">
        <li class="page-item"><a class="page-link" href="GuestList.gu?pag=1">◀◀</a></li>
      </c:if>
      <c:if test="${curBlock > 0}">
        <li class="page-item"><a class="page-link" href="GuestList.gu?pag=${curBlock * blockSize}">◁</a></li>
      </c:if>
    	<c:forEach var="i" begin="${curBlock * blockSize + 1}" end="${curBlock * blockSize + blockSize}" varStatus="St">
        <c:if test="${i <= totPage && i == pag}"><li class="page-item"><a class="page-link" href="GuestList.gu?pag=${i}" style="color:red;font-weight:bold;">${i}</a></li></c:if>
        <c:if test="${i <= totPage && i != pag}"><li class="page-item"><a class="page-link" href="GuestList.gu?pag=${i}">${i}</a></li></c:if>
      </c:forEach>
      <c:if test="${curBlock < lastBlock}">
        <li class="page-item"><a class="page-link" href="GuestList.gu?pag=${curBlock * blockSize + blockSize + 1}">▷</a></li>
      </c:if>
      <c:if test="${pag < totPage}">
        <li class="page-item"><a class="page-link" href="GuestList.gu?pag=${totPage}">▶▶</a></li>
      </c:if>
    </ul>
    <!-- 블록페이지 끝 -->
